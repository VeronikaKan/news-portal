...
async function loginUser(req, res) {
  ...
  try {
    ...
    if (user.is_password_recovery) {
      return res
        .status(401)
        .json({ message: "must create new password", userId: user.user_id });
    }
...
async function setNewPassword(req, res) {
  ...
  try {
    ...
    if (!user.is_password_recovery) {
      return res
        .status(401)
        .json({ message: "must login", userId: user.user_id });
    }
    ...
    return res
      .status(201)
      .json({
        message: `new password for user - ${email} is successfully changed`,
        userId: user.user_id,
      });
  } catch (err) {
    console.log(err);
